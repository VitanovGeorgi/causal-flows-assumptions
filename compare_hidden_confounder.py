...
if cfg.dataset.name in ["german"]:
    ...
    def run_experiments(cfg_1, cfg_2, hidden_confounder_idx: int, output_folder: str = None):
        ...
        # creating model_2 data with model_1 data !!!!
        model_2_preparator = SCMPreparator.loader(cfg_2.dataset)
        X_2, U_2 = model_2_preparator.prepare_data(True)

        """ Hardcoding which features are deleted, and hot to rearrange the rest.
        Replacing the original data of model_2 with the data of model_1.
        """
        for dataset_1, dataset_2 in zip(model_1_preparator.datasets, model_2_preparator.datasets):
            dataset_2.X = dataset_1.X[:, [1, 2, 3]]
            dataset_2.U = dataset_1.U[:, [1, 2, 3]]
        pdf_2, corr_2 = model_2_preparator.get_features_all()
        
        model_2_loaders = model_2_preparator.get_dataloaders(
            batch_size=cfg_2.train.batch_size, num_workers=cfg_2.train.num_workers
        )
        model_2 = causal_nf_train.load_model(cfg=cfg_2, preparator=model_2_preparator)
        run_uuid = str(uuid.uuid1()).replace("-", "")
        
        dirpath = os.path.join(cfg_2.root_dir, run_uuid)
        logger_dir = os.path.join(cfg_2.root_dir, run_uuid)
        
        model_2_trainer, model_2__logger = causal_nf_train.load_trainer(
            cfg=cfg_2,
            dirpath=dirpath,
            logger_dir=logger_dir,
            include_logger=True,
            model_checkpoint=cfg_2.train.model_checkpoint,
            cfg_early=cfg_2.early_stopping,
            preparator=model_2_preparator,
        )

       
        model_2_trainer.fit(model_2, train_dataloaders=model_2_loaders[0], val_dataloaders=model_2_loaders[1])
        ckpt_name_list = ["last"]
        
        if cfg_2.early_stopping.activate:
            ckpt_name_list.append("best")
        
        for ckpt_name in ckpt_name_list:
            for i, loader_i in enumerate(model_2_loaders):
                s_name = model_2_preparator.split_names[i]
                causal_nf_io.print_info(f"Testing {s_name} split")
                model_2_preparator.set_current_split(i)
                model_2.ckpt_name = ckpt_name
                _ = model_2_trainer.test(ckpt_path=ckpt_name, dataloaders=loader_i)
                metrics_stats = model_2.metrics_stats
                metrics_stats["current_epoch"] = model_2_trainer.current_epoch

        X_2_obs = model_2.input_scaler.inverse_transform(
            model_2.model.sample((X_2.shape[0],))['x_obs'], inplace=False
        )

        U_2_obs = model_2.input_scaler.inverse_transform(
            model_2.model.sample((U_2.shape[0],))['u_obs'], inplace=False
        )
    

        # creating model_2 data with model_2 data
        model_3_preparator = SCMPreparator.loader(cfg_2.dataset)
        X_3, U_3 = model_3_preparator.prepare_data(True)

        pdf_3, corr_3 = model_3_preparator.get_features_all()
        
        model_3_loaders = model_3_preparator.get_dataloaders(
            batch_size=cfg_2.train.batch_size, num_workers=cfg_2.train.num_workers
        )
        model_3 = causal_nf_train.load_model(cfg=cfg_2, preparator=model_3_preparator)
        run_uuid = str(uuid.uuid1()).replace("-", "")
        
        dirpath = os.path.join(cfg_2.root_dir, run_uuid)
        logger_dir = os.path.join(cfg_2.root_dir, run_uuid)
        
        model_3_trainer, model_3_logger = causal_nf_train.load_trainer(
            cfg=cfg_2,
            dirpath=dirpath,
            logger_dir=logger_dir,
            include_logger=True,
            model_checkpoint=cfg_2.train.model_checkpoint,
            cfg_early=cfg_2.early_stopping,
            preparator=model_3_preparator,
        )

       
        model_3_trainer.fit(model_3, train_dataloaders=model_3_loaders[0], val_dataloaders=model_3_loaders[1])
        ckpt_name_list = ["last"]
        ...
        for ckpt_name in ckpt_name_list:
            for i, loader_i in enumerate(model_3_loaders):
                s_name = model_3_preparator.split_names[i]
                causal_nf_io.print_info(f"Testing {s_name} split")
                model_3_preparator.set_current_split(i)
                model_3.ckpt_name = ckpt_name
                _ = model_3_trainer.test(ckpt_path=ckpt_name, dataloaders=loader_i)
                metrics_stats = model_3.metrics_stats
                metrics_stats["current_epoch"] = model_3_trainer.current_epoch

        X_3_obs = model_3.input_scaler.inverse_transform(
            model_3.model.sample((X_3.shape[0],))['x_obs'], inplace=False
        )

        U_3_obs = model_3.input_scaler.inverse_transform(
            model_3.model.sample((U_3.shape[0],))['u_obs'], inplace=False
        )
        ...
        # be mindful of column order!
        df_x1_obs = pd.DataFrame.from_dict({
            'x1': X_1_obs[:, 1].detach().numpy(),
            'x2': X_1_obs[:, 2].detach().numpy(),
            'x3': X_1_obs[:, 3].detach().numpy(),
            'type': f"x_obs[1:3] ~ p_theta_{cfg_1.dataset['name']}({cfg_2.dataset['name']})"
        })

        df_x2_obs = pd.DataFrame.from_dict({
            'x1': X_2_obs[:, 0].detach().numpy(),
            'x2': X_2_obs[:, 1].detach().numpy(),
            'x3': X_2_obs[:, 2].detach().numpy(),
            'type': f"x_obs ~ p_theta_{cfg_2.dataset['name']}({cfg_1.dataset['name']}[1:3])"
        })

        df_x3_obs = pd.DataFrame.from_dict({
            'x1': X_3_obs[:, 0].detach().numpy(),
            'x2': X_3_obs[:, 1].detach().numpy(),
            'x3': X_3_obs[:, 2].detach().numpy(),
            'type': f"x_obs ~ p_theta_{cfg_2.dataset['name']}({cfg_2.dataset['name']})"
        })

        df_x1 = pd.DataFrame.from_dict({
            'x1': X_1[:, 1].detach().numpy(),
            'x2': X_1[:, 2].detach().numpy(),
            'x3': X_1[:, 3].detach().numpy(),
            'type': f"x_true[1:3] ~ p_{cfg_1.dataset['name']}({cfg_1.dataset['name']})"
        })

        df_x2 = pd.DataFrame.from_dict({
            'x1': X_2[:, 0].detach().numpy(), # same as X_3 == X_2
            'x2': X_2[:, 1].detach().numpy(),
            'x3': X_2[:, 2].detach().numpy(),
            'type': f"x_true ~ p_{cfg_1.dataset['name']}({cfg_1.dataset['name']})"
        })

        df_u1_obs = pd.DataFrame.from_dict({
            'u1': U_1_obs[:, 1].detach().numpy(),
            'u2': U_1_obs[:, 2].detach().numpy(),
            'u3': U_1_obs[:, 3].detach().numpy(),
            'type': f'oracle u_obs'
        })

        df_u2_obs = pd.DataFrame.from_dict({
            'u1': U_2_obs[:, 0].detach().numpy(),
            'u2': U_2_obs[:, 1].detach().numpy(),
            'u3': U_2_obs[:, 2].detach().numpy(),
            'type': f'empirical u_obs'
        })

        df_u3_obs = pd.DataFrame.from_dict({
            'u1': U_3_obs[:, 0].detach().numpy(),
            'u2': U_3_obs[:, 1].detach().numpy(),
            'u3': U_3_obs[:, 2].detach().numpy(),
            'type': f'empirical true u_obs'
        })

        sns_plot_1 = sns.pairplot(pd.concat([df_x1_obs, df_x2_obs, df_x3_obs], axis=0), hue='type')
        sns_plot_1.map_offdiag(sns.kdeplot, levels=4, color=".1")

        sns_plot_1.figure.subplots_adjust(top=0.95) 
        sns_plot_1.figure.suptitle(f"p_theta_confounded v. p_theta_unconfounded(confounded_data) v. p_theta_unconfounded")
        fig_name =f"{cfg_1.dataset['name']}_{cfg_2.dataset['name']}_{cfg.dataset['num_samples']}_1.png"
        sns_plot_1.figure.savefig(
            os.path.join(output_folder, fig_name)
        )

        sns_plot_2 = sns.pairplot(pd.concat([df_x1, df_x2], axis=0), hue='type')
        sns_plot_2.map_offdiag(sns.kdeplot, levels=4, color=".1")

        sns_plot_2.figure.subplots_adjust(top=0.95) 
        sns_plot_2.figure.suptitle(f"Data generation process: {cfg_1.dataset['name']} (confounded) v. {cfg_2.dataset['name']} (unconfounded)")
        fig_name =f"{cfg_1.dataset['name']}_{cfg_2.dataset['name']}_{cfg.dataset['num_samples']}_2.png"
        sns_plot_2.figure.savefig(
            os.path.join(output_folder, fig_name)
        )

        sns_plot_3 = sns.pairplot(pd.concat([df_x1_obs, df_x2_obs, df_x1], axis=0), hue='type')
        sns_plot_3.map_offdiag(sns.kdeplot, levels=4, color=".1")

        sns_plot_3.figure.subplots_adjust(top=0.95) 
        sns_plot_3.figure.suptitle(f"Fit of confounded model v. uncounfounded model on confounded data v. true data generated by confounded model")
        fig_name =f"{cfg_1.dataset['name']}_{cfg_2.dataset['name']}_{cfg.dataset['num_samples']}_3.png"
        sns_plot_3.figure.savefig(
            os.path.join(output_folder, fig_name)
        )

        sns_plot_4 = sns.pairplot(pd.concat([df_x3_obs, df_x2], axis=0), hue='type')
        sns_plot_4.map_offdiag(sns.kdeplot, levels=4, color=".1")

        sns_plot_4.figure.subplots_adjust(top=0.95) 
        sns_plot_4.figure.suptitle(f"How well uncounfounded model 2 would have fitted itself, p_unconfounded_model || p_theta_uncounfounded_model")
        fig_name =f"{cfg_1.dataset['name']}_{cfg_2.dataset['name']}_{cfg.dataset['num_samples']}_4.png"
        sns_plot_4.figure.savefig(
            os.path.join(output_folder, fig_name)
        )
