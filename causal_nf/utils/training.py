...
def load_normalizing_flow(cfg, preparator, ckpt_file=None):
    ...
    if cfg.model.adjacency:
        adjacency = preparator.adjacency() # get's the graph from the preparator
    else:
        adjacency = None
    ...
    if model_name == "causal_nf":
        ...
        if flow_name == "maf":
            flow = zflows.MAF(
                dim_x,
                0,
                transforms=num_layers, # L in paper == length of sequence of transforms of NF, keep in mind transofr is a NN with diff no layers
                # num_layer != num_layer in NN, but no transforms in NF
