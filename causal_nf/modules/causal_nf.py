...
class CausalNormalizingFlow(nn.Module):
    ...
    def compute_counterfactual_2(
        ...
        """
            Do and interventions, by simpply chaning the adjacency matrix, and then replacing all vals 
            besides needed to 0, and needed to alpha
        """
        n_flow = self.flow()
    ...
    def intervene(
        ...
        """
            Do-operator, just take the Pu which give you the P(do)
        """
        n_flow = self.flow()
        if scaler is not None:  # x is not normalized
            n_flow.transform = ComposedTransform(scaler, n_flow.transform)
        ...
        # Get distributional x1
        """
            Samples in data from Pu, by giving samples from Pu
        """
        x1 = n_flow.transform.inv(z1)
        ...
        # Get interventional z
        """
            Find from which Pu these would have been drawn
        """
        z2 = n_flow.transform(x1)
        """
            Replace only those where index was, so modified pdf should only affect the children of the index
        """
        z2[:, index + 1 :] = z1[:, index + 1 :]
        """
            Resample 'intervened' data
        """
